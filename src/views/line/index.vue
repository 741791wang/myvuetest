  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      >
        ...
        <el-table-column align="center" label="生产线编号">
          <template slot-scope="scope">
            {{ scope.row.plineId }}
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="130px" style="width: 800px; margin-left:50px;">
        <el-form-item prop="plineId" label="生产线编号" style="width: 150px">
          <el-input v-model="temp.plineId" :disabled="dialogStatus === 'update'" placeholder="请输入" style="width: 270px" />
        </el-form-item>
...
import { getList, getProductList, createLine, updateLine, deleteLine } from '@/api/line'
...
export default {
  ...
  data() {
    return {
      ...
      goodList: [],
      rules: {
        plineId: [{ required: true, message: '工位编号不为空', trigger: 'blur' }],
        goodName: [{ required: true, message: '工位名称不为空', trigger: 'blur' }]
      },
      dialogStatus: '',
      textMap: {
        update: '修改',
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var tempData = {
            plineId: this.temp.plineId,
            reMark: this.temp.remark
          }
          createLine(tempData).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateLine(flagdata).then(response => {
            if (response.data === true) {
              this.fetchdata()
              this.dialogFormVisible = false
              this.$notify({
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        deleteLine(row.plineId).then(response => {
          if (response.data === true) {
            this.$message({
              type: 'success',
              message: '删除成功!'
